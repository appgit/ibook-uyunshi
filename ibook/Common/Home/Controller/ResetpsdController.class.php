...
class ResetpsdController extends Controller {
    ...
    public function resetpasswd(){
            ...
            if(!$user){
               ...
               $is_mail_ok = sendEmail($user['username'],$email,$verify,true);

               if($is_mail_ok){

                $this->show("<h1>重置密码的链接已发送至您的邮箱，快去邮箱看看吧</h1><a target='_blank' href='http://".$email_href."' style='font-size:20px;'>点击这里</a></h1><br />");

               }else{

                $this->show('<h1>:(  对不起，邮件发送失败</h1>');
               }
            }
    }
