...
class ProductsController extends Controller {
	...
	public function index(){
		...
		$from_page = $_SERVER['HTTP_REFERER'];	/*没从正确入口进来的孩纸*/

		if (current(I('get.')) === false) {

			redirect(U('Products/index', array('sp' => "文化书籍"),''),0,'直接访问了该操作,给个默认地址');

		}
			
			$list_num = 7;/*每页显示数量*/

			if(cookie('_view_grid')){
	...
	public function grid(){
		...
		if($_SERVER['HTTP_REFERER']!=null){
			...
			if($_GET['show'] == 'true'){
				...
				cookie('_view_grid',true,3600*24*30);
				cookie('_view_grid_active','active',3600*24*30);
			}else{
				cookie('_view_grid',null);
				cookie('_view_grid_active',null);
			}
...
/*特价*/
	public function bargin(){
		...
		/*类别按钮*/
		$this->assign('type_url1',__CONTROLLER__.'/bargin/type/2'.$url_argv_base);
		$this->assign('type_url2',__CONTROLLER__.'/bargin'.$url_argv_base);
		$this->assign('type_url3',__CONTROLLER__.'/bargin/type/1'.$url_argv_base);
		...
		/*排序按钮的url*/
		$this->assign('sortby_latest',__CONTROLLER__.'/bargin'.$url_argv.'/sortby/latest'.$this_page);
		$this->assign('sortby_price',__CONTROLLER__.'/bargin'.$url_argv.'/sortby/price'.$this_page);
		$this->assign('sortby_chat',__CONTROLLER__.'/bargin'.$url_argv.'/sortby/chat'.$this_page);
		$this->assign('sortby_default',__CONTROLLER__.'/bargin'.$url_argv.'/sortby/default'.$this_page);

		/*分页除page参数外的url参数*/
		$this->assign('base_url',__CONTROLLER__.'/bargin'.$url_argv.$sortby_url);
		$this->assign('errormsg',$errormsg);
		$this->display('index');
	}
