...
/*公共初始化函数*/
function common_initial(){

	autologin();	//自动登录检查
	notread_order();		//未读约谈数量

}


/*未读订单数*/
function notread_order(){

	$sql['sid'] = session('_userinfoid');

    $sql['isread'] = 0;

    $notread_num = count(M('orders')->where($sql)->select());

    if( $notread_num > 0 ){
    	
    	session('_not_read_chat_num',$notread_num);

    }else{

    	session('_not_read_chat_num',null);
    }
...
//发送邮件
function sendEmail($username,$email,$activecode,$type){

	#type:0 注册 -- type:1 重置密码 -- type:2 发送订单请求
	...
	$emailbody = bat_str_replace($arr,$html);
		...
		if( $type == 1 ){		//发送忘记密码邮件
			...
			$arr = array(
						...
						'$_HOME'	=> __DOMIN__.__MODULE__,

						'$_GOODS'	=> $activecode['name'],

						'$_ID'	=> $activecode['id']
					);

			$html = file_get_contents(T('Mailhtml/order'));
	...
	if (cookie('_verify')) {	//自动登录检查
		...
		if( $autologin['token_exptime'] > $time ){
			...
			$arr = array(   //session 信息
				...
				'_lastlogindate' => $userinfo['lastlogindate']
			);
